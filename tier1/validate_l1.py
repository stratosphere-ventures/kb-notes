...
# Rule checks
# -----------------------------
def check_governance(day: Dict[str, Any], errors: List[LintItem], warnings: List[LintItem], filename: str):
    ...
    for k in required:
        if k not in g:
            add_item(errors, "L1-GOV-001", "HARD", f"governance.{k}", f"Missing required governance field '{k}'.")
    ...
    if pub and not ISO_Z_RE.match(pub):
        add_item(errors, "L1-GOV-002", "HARD", "governance.published_ts", "published_ts must be ISO8601 Zulu (YYYY-MM-DDTHH:MM:SSZ).")
    if asof and not ISO_Z_RE.match(asof):
        add_item(errors, "L1-GOV-002", "HARD", "governance.asof_ts", "asof_ts must be ISO8601 Zulu (YYYY-MM-DDTHH:MM:SSZ).")
...
def check_market_outcomes(day: Dict[str, Any], errors: List[LintItem], warnings: List[LintItem]):
    ...
    for i, idx in enumerate(indices):
        ...
        if prev is not None and close is not None and ret is not None and prev != 0:
            ...
            if abs(calc - ret) > 0.02:
                add_item(errors, "L1-CON-002", "HARD", f"market_outcomes.indices[{i}]", f"{sym}: return_pct inconsistent with close/prev_close (calc={calc:.3f}, got={ret:.3f}).")
    ...
    for i, r in enumerate(rfc):
        unit = r.get("unit")
        sym = r.get("symbol", "")
        if unit not in ("pct","bps","usd","index","other"):
            add_item(errors, "L1-CON-003", "HARD", f"market_outcomes.rates_fx_commodities[{i}].unit", f"Unknown unit '{unit}' for {sym}.")

def check_events_reactions_evidence(day: Dict[str, Any], errors: List[LintItem], warnings: List[LintItem]):
    ...
    for i, e in enumerate(events):
        ...
        for j, f in enumerate(facts):
            ...
            if bad:
                add_item(errors, "L1-PUR-001", "HARD", f"events[{i}].facts[{j}]", f"Event fact contains forbidden token '{bad}'.")
    ...
    for i, rw in enumerate(rws):
        ...
        if eid and eid not in set(event_ids):
            add_item(errors, "L1-REF-002", "HARD", f"reaction_windows[{i}].event_id", f"reaction_windows references missing event_id '{eid}'.")
    ...
    for i, ev in enumerate(evs):
        eid = ev.get("event_id")
        if eid and eid not in set(event_ids):
            add_item(errors, "L1-REF-003", "HARD", f"evidence_items[{i}].event_id", f"evidence_items references missing event_id '{eid}'.")
        ...
        if not re.search(r"\d", excerpt) and not re.search(r"\b(announced|reported|filed|released|approved|priced)\b", excerpt.lower()):
            add_item(warnings, "L1-PUR-002", "SOFT", f"evidence_items[{i}].excerpt", "Evidence excerpt may not be fact-style (no digits/action verbs).")

def cross_file_realism(days: List[Tuple[str, Dict[str, Any]]], errors: List[LintItem], warnings: List[LintItem], eval_mode: bool):
    """
...
def main():
    ...
    for f in files:
        ...
        try:
            ...
            reports.append(FileReport(filename=f, schema_ok=False, errors=errors, warnings=warnings))
            continue
        ...
        reports.append(FileReport(filename=f, schema_ok=ok, errors=errors, warnings=warnings))

    # cross-file realism checks
    cross_errors: List[LintItem] = []
    cross_warnings: List[LintItem] = []
    cross_file_realism(loaded_days, cross_errors, cross_warnings, eval_mode=(args.profile=="eval"))
    ...
    for r in reports:
        ...
        status = "OK" if (not r.errors and (args.profile!="eval" or not r.warnings)) else "FAIL"
    ...
    def to_dict_item(it: LintItem) -> Dict[str, Any]:
        return {"code": it.code, "severity": it.severity, "path": it.path, "message": it.message}
